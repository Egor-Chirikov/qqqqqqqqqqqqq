...
int main() {
    ...
    if (term != '\n' || str <= 0 || str >= N)                                                  //    !!!!!! str >= N  || str > N убрал дублируешие условие
    {
        printf("Указана неверная длина строк\n");
        return 0;
    }
    ...
    if (term != '\n' || col <= 0 || col >= M)                                                 //    !!!!!! col > M || col >= M убрал дублируешие условие
    {
        printf("Указана неверная длина столбцов\n");
        return 0;
    }
    ...
    /*                                                                                              //!!!! получется ты выводишь матрицу без последний строки, а не удалешь строку.
    // Проверка на то, является ли нужная строка последней и не единственной строкой в матрице      //!!!! Ларина это не зачтёт. Да и кому же потом ты её удалишь(стр 135) ps. строку, не Ларину
    ...
    */
    ...
    if (min_str == str - 1 && str == 1)
    {   
       //str -= 1;                                                                                 // !!! это не имеет смысла делать. Так как программа завершает работу
